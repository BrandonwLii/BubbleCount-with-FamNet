...
def crop_images(image_path1, image_path2, crop_size, crop_position, output_path):
    ...
    # Crop the images
    cropped_image1 = image1.crop((crop_position[0], crop_position[1],
                                  crop_position[0] + crop_size[0], crop_position[1] + crop_size[1]))
    cropped_image2 = image2.crop((crop_position[0], crop_position[1],
                                  crop_position[0] + crop_size[0], crop_position[1] + crop_size[1]))
...
def insert_cropped(sample_img, target_img, boxes, num_box=4):
    ...
    for i, box in enumerate(boxes):
        ...
        if i == 0:
            position = (hybrid_img.width - cropped_section.width, hybrid_img.height - cropped_section.height)  # Bottom right corner
        elif i == 1:
            position = (hybrid_img.width - cropped_section.width, 0)  # Top right corner
        elif i == 2:
            position = (0, hybrid_img.height - cropped_section.height)  # Bottom left corner
        elif i == 3:
            position = (0, 0)  # Top left corner
        elif i == 4:
            position = (0, hybrid_img.height//2)  # Middle of left edge
        elif i == 5:
            position = (hybrid_img.width - cropped_section.width, hybrid_img.height//2)
        
        # Paste the cropped section into the target image
        hybrid_img.paste(cropped_section, position)

        inserted_position = [position[1], position[0], position[1] + cropped_section.height, position[0] + cropped_section.width]
